...
between experience-sharing methods (SUPER~\cite{gerstgrasser2023}) and
weight-sharing methods (FedAvg~\cite{mcmahan2017}).  We call this the
...
The closest conceptual work is Cook et al.'s cultural accumulation
framework~\cite{cook2024}, which demonstrates generational knowledge transfer
via PPO with a behavioral cloning auxiliary loss.  Each generation trains on
demonstrations from the previous generation, producing a ``cultural ratchet.''
Our approach differs by sharing distilled \emph{value functions} rather than
raw behavioral demonstrations, and operates bidirectionally across the entire
population rather than unidirectionally from parent to child.

\paragraph{Experience sharing.}
SUPER~\cite{gerstgrasser2023} (NeurIPS 2023) shares full experience tuples
$(s, a, r, s')$ between agents in cooperative MARL.
ERL/CERL~\cite{pourchot2019} inject gradient-free evolutionary actors into
a shared replay buffer alongside RL actors.  The morphic field shares at a
higher level of abstraction: entire Q-tables (value functions) rather than
individual transitions, producing more compact transfer.

\paragraph{Federated RL.}
FEDORA~\cite{che2023} applies federated learning directly to RL, aggregating
policy parameters across distributed agents.
FedAvg~\cite{mcmahan2017} averages model weights uniformly.  The morphic
field differs in two key ways: (1)~contributions are weighted by
\emph{performance}, not uniformly averaged; (2)~temporal decay
(Section~\ref{sec:architecture:field}) allows the field to forget stale
knowledge, a mechanism absent from all existing FedRL methods.
...
spectrum: between experience-sharing methods (SUPER, ERL/CERL) that share
raw transitions, and weight-sharing methods (FedAvg, FEDORA) that share full
model parameters.  The morphic field shares \emph{value functions} ---
...
\subsection{Bioelectric Morphogenetic Fields}
\label{sec:background:levin}

Michael Levin's laboratory at Tufts University has demonstrated that
\emph{real} morphogenetic fields exist at the cellular level --- bioelectric
signal patterns that guide development, regeneration, and collective cell
behavior~\cite{levin2025, manicka2025}.

\paragraph{Manicka \& Levin 2025.}
In a computational study using Active Inference~\cite{friston2010} --- the
same framework used in GraphPalace's agent architecture --- Manicka and
Levin~\cite{manicka2025} show how bioelectric networks create fields that
cells ``read'' to make developmental decisions.  The mechanism is structurally
identical to our morphic field: cells emit bioelectric signals $\rightarrow$
signals accumulate in the tissue $\rightarrow$ new cells read the accumulated
field to decide their fate.  This is absorb/emit/blend at the cellular level.

\paragraph{Levin 2025.}
In ``The Multiscale Wisdom of the Body''~\cite{levin2025}, Levin argues that
biological intelligence operates at multiple scales: molecular $\rightarrow$
cellular $\rightarrow$ tissue $\rightarrow$ organism, with dynamic rewritable
memory at each level.  The morphic field in our simulation operates at the
same abstraction: individual agent $\rightarrow$ population $\rightarrow$
field $\rightarrow$ new agent.

\paragraph{Significance.}
Levin's work provides a mainstream scientific bridge between Sheldrake's
controversial hypothesis and our engineering implementation.  Bioelectric
morphogenetic fields are published in \emph{Nature}, \emph{Cell}, and
\emph{BioEssays} --- they are not fringe science.  The morphic field in our
simulation can be understood as a computational analogue of bioelectric
patterning, independent of whether one endorses Sheldrake's metaphysics.
...
Alfred North Whitehead's process philosophy~\cite{whitehead1929} provides
the most precise philosophical mapping to our morphic field architecture.
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{Prehension $\rightarrow$ emission.}
          ...
          \texttt{field.emit()} --- the new agent grasps accumulated
...
This is not a loose analogy but a structural isomorphism.  Whitehead's
metaphysics and our computational architecture share the same data flow:
entities arise, contribute to a shared medium, perish, and inform future
entities through that medium.
...
than ancient ones, preventing stale knowledge from dominating.  This is the
key differentiator from federated RL methods, none of which implement temporal
decay.  Section~\ref{sec:results:decay} evaluates the effect of different
decay rates.
...
0.01, 0.02, 0.05\}$) to determine whether temporal decay improves performance.
Moderate decay ($\rho = 0.01$--$0.02$) prevents stale early-generation
knowledge from diluting recent improvements, while high decay ($\rho = 0.05$)
forgets too aggressively.  The optimal decay rate balances recency with
cumulative depth --- precisely the trade-off that pheromone systems
solve in biological stigmergy.

% -- 4.4 Phase Transition ---------------------------------------------------
\subsection{Phase Transition: Population Threshold}
\label{sec:results:phase}

We vary population size ($P \in \{1, 2, 5, 10, 20, 40\}$) while keeping all
other parameters constant to test whether the morphic field exhibits a phase
transition --- a critical population threshold below which the field fails to
form coherent knowledge.  This experiment connects to Khushiyant's (2025)
finding of phase transitions in stigmergic collective memory at critical
density $\rho \approx 0.20$--$0.23$~\cite{khushiyant2025}.

% -- 4.5 Adversarial Robustness ---------------------------------------------
\subsection{Adversarial Robustness}
\label{sec:results:adversarial}

We test the morphic field's resilience to adversarial Q-value injection by
comparing an undefended \texttt{MorphicField} against a
\texttt{RobustMorphicField} with consensus filtering.  Three attack types are
tested at the midpoint of training: (1)~random noise injection with high
performance weight, (2)~degenerate policy injection (always-left Q-tables),
and (3)~zero-flood attacks that dilute the field.  The consensus filter
rejects Q-tables whose mean deviation from the running field statistics
exceeds a threshold, preventing adversarial contributions from corrupting
the accumulated knowledge.

% -- 4.6 Multi-Maze Generalization ------------------------------------------
\subsection{Multi-Maze Generalization}
\label{sec:results:multimaze}

We test whether the morphic field captures general maze-solving strategies
or merely memorizes Q-values for a specific topology.  Two experiments:
(1)~\emph{Same-size transfer}: build the field on Maze~A (seed=42) and test
displaced agents on Mazes B, C, D (different seeds, same size);
(2)~\emph{Cross-maze field}: train across four mazes simultaneously,
alternating each generation, then test on all four plus a held-out maze.

% -- 4.7 Deep RL Extension --------------------------------------------------
\subsection{Deep RL Extension}
\label{sec:results:deep}

To demonstrate that the morphic field pattern scales beyond tabular
Q-learning, we implement a numpy-only DQN (2-layer neural network,
64 hidden units, experience replay) and test the morphic field as
shared network weight initialization.  The
\texttt{DeepMorphicField} accumulates and emits network parameters
($\mathbf{W}_1, \mathbf{b}_1, \mathbf{W}_2, \mathbf{b}_2$) using
the same performance-weighted absorb/emit cycle.
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Scale.}  While the deep RL extension
          (Section~\ref{sec:results:deep}) demonstrates the pattern scales
          beyond tabular Q-learning, it uses a small numpy-only DQN.
          Production validation with PyTorch/TensorFlow on continuous
          state spaces remains future work.
    ...
    \item \textbf{No real biology.}  The simulation models artificial agents,
          ...
          loose at best, though Levin's bioelectric field
          work~\cite{levin2025} provides a biological bridge.
    ...
    \item \textbf{Single seed.}  Primary results are from a single random
          seed.  The multi-maze experiments
          (Section~\ref{sec:results:multimaze}) test across topologies,
          but a full study would require multiple seeds with bootstrap
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{Production-scale deep RL}: extend the numpy DQN proof of
          ...
          (Atari, MuJoCo) with proper target networks and prioritized replay.
    ...
    \item \textbf{Head-to-head FedRL comparison}: benchmark morphic field
          directly against FedAvg~\cite{mcmahan2017},
          FEDORA~\cite{che2023}, and SUPER~\cite{gerstgrasser2023} on
          identical cooperative tasks with identical compute budgets.
    \item \textbf{LLM agent fields}: apply the morphic field pattern to
          LLM tool-use strategy distributions, where agents share
          accumulated knowledge about which tools to invoke for which tasks.
\end{enumerate}
...
experimental axes.  The key findings:
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item The displaced control test produces a distinctive signature: fresh
          ...
          habit.
    \item Temporal decay ($\rho = 0.01$--$0.02$) improves field quality
          by preventing stale knowledge from dominating --- a mechanism
          absent from all existing FedRL methods.
    \item The morphic field exhibits population-dependent effectiveness,
          with a clear threshold below which the field fails to form
          coherent knowledge.
    \item Consensus filtering provides robustness against adversarial
          Q-value injection, with up to 14\% recovery against zero-flood
          attacks.
    \item The morphic field architecture extends to neural Q-networks and
          transfers partially across maze topologies, demonstrating
          generality beyond the tabular setting.
\end{enumerate}

The hypothesis is formally coherent, the predicted data pattern is distinctive
and detectable, and the displaced control constitutes a viable experimental
design.  The simulation does not prove that morphic resonance exists in
nature --- it is a computational thought experiment that makes the hypothesis
precise enough to test empirically.

The engineering contribution stands independently.  The morphic field
pattern --- value-function-level sharing with performance weighting,
temporal decay, and consensus filtering --- occupies a specific gap between
experience-sharing (SUPER) and weight-sharing (FedAvg) in the multi-agent
knowledge transfer spectrum.  The insight that learning should not die with
the agent --- that collective memory can be infrastructure --- provides a
clean, minimal implementation with a direct path from biological inspiration
to practical multi-agent architecture.

\vspace{1em}
\noindent\textbf{Code availability.}
The simulation code, experiment scripts, and paper source are available at
...
\bibitem{cook2024}
T.~Cook, S.~Lu, E.~Grefenstette, and J.~Foerster, ``Artificial generational
intelligence: Cultural accumulation in reinforcement learning,''
\emph{arXiv:2406.00392}, 2024.

\bibitem{gerstgrasser2023}
M.~Gerstgrasser, T.~Danino, and S.~Koenig, ``SUPER: Sharing useful
partial experiences in multi-agent reinforcement learning,'' in
\emph{Proc.\ NeurIPS}, 2023.

\bibitem{pourchot2019}
A.~Pourchot and O.~Sigaud, ``CEM-RL: Combining evolutionary and
gradient-based methods for policy search,'' in \emph{Proc.\ ICLR}, 2019.

\bibitem{che2023}
J.~Che, M.~Bedi, and A.~Koppel, ``FEDORA: Federated reinforcement
learning with online aggregation,'' \emph{arXiv:2312.05035}, 2023.

\bibitem{levin2025}
M.~Levin, ``The multiscale wisdom of the body,'' \emph{BioEssays}, 2025.

\bibitem{manicka2025}
S.~Manicka and M.~Levin, ``Field-mediated bioelectric basis of morphogenetic
prepatterning,'' \emph{bioRxiv}, 2025.

\bibitem{friston2010}
K.~Friston, ``The free-energy principle: A unified brain theory?''
\emph{Nature Reviews Neuroscience}, vol.~11, pp.~127--138, 2010.

\bibitem{khushiyant2025}
Khushiyant, ``Emergent collective memory in stigmergic systems,''
\emph{arXiv:2512.10166}, 2025.
